...
class UserList(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.user.role is not None and request.user.role in ['owner']:
            ...
            return Response(User.objects \
                            ...
                            .values('id', 'first_name', 'last_name', 'role',
                                    'facility__name', 'facility__address')
                            .distinct('id'))
...
class UserDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not _has_role_permissions(request.data['role']):
            return Response('({}) user has no permission to delete {}'.format(
                request.user.role or 'Simple',
                request.data['role'].title()
            ),
                status=status.HTTP_403_FORBIDDEN)
