...
class FacilityCompleteness(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not FacilityUtils.has_rights(
            request.user.role,
            ["admin", "owner", "coordinator", "case_manager", "network_manager", "facility_manager",],
        ):
...
class FacilityAddChild(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        return Response("Child: {} was added to facility: {}".format(child_id, facility_id), status=status.HTTP_200_OK,)
...
class FacilityAddManager(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if facility_id is None:
            return Response("We should pass a valid facility id", status=status.HTTP_404_NOT_FOUND)
        if user_id is None:
            return Response("We should pass a valid user id", status=status.HTTP_404_NOT_FOUND)
        ...
        if str(user.facility_id) == facility_id:
            return Response(
                "User id {} already belongs to the facility id {}".format(user_id, facility_id),
        ...
        return Response("User: {} is assigned to facility: {}".format(user_id, facility_id), status=status.HTTP_200_OK,)
...
class FacilityRemoveManager(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user.save()
        return Response(
            "User: {} is removed from facility: {}".format(user_id, facility_id), status=status.HTTP_200_OK,
        )
