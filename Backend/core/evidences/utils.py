...
def evidence_image_path(instance, filename):
    path = 'evidence_images/'
    path = path + '%s' % instance.evidence_image
    return path


def save_evidence_image(image, user):
    try:
        ...
        thumb_file = InMemoryUploadedFile(ContentFile(thumb_io.getvalue()), None, img_name, 'image/jpeg', thumb_io.len, None)
        return thumb_file
